    ...
    public class DbScript
    {
        ...
        public string Load()
        {
            switch (ScriptType)
            {
                case ScriptType.File:
                {
                    ...
                    if (Buddy.Utility.Path.IsRelative(fileName))
                    {
                        fileName = Path.Combine(GetBaseDirectory.Invoke(), ScriptValue);
                    }

                    if (!Buddy.Utility.Path.FileExists(fileName))
                    {
        ...
        public string XmlRoot
        {
            set
            {
                ...
                if (!System.Enum.TryParse(root.Attribute("type").Value, false, out scriptType))
