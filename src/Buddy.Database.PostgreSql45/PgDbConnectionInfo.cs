...
namespace Buddy.Database.PostgreSql
{
    internal sealed class PgDbConnectionInfo : DbConnectionInfo
    {
        ...
        internal const string UserNameKey = "username"; // Preferred
        internal const string UserIdKey = "user id"; // Legacy
        ...
        public override string UserName
        {
            get
            {
                if (connectionStringBuilder.ContainsKey(UserNameKey))
                    return (string)connectionStringBuilder[UserNameKey];

                if (connectionStringBuilder.ContainsKey(UserIdKey))
                    return (string)connectionStringBuilder[UserIdKey];
